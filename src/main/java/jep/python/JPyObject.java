...
public class JPyObject {
    ...
    public JPyObject(long tstate, long pyObject, Jep jep) {
        this.tstate = tstate;
     ...
     *             if an error occurs
     */
    protected void isValid() throws JepException {
        if (pyObject == 0) {
            throw new JepException(super.toString() + " invalid pointer.");
        }
        jep.isValidThread();
    }
    ...
    public Object getAttr(String name) throws JepException {
        isValid();
        return getAttr(tstate, pyObject, name);
    }
